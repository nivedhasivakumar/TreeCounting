...
tf.flags.DEFINE_string("data_dir", "/notebooks/tree_counting/Data", "path to dataset")
...
def main(argv=None):
    ...
    if FLAGS.mode == "train":
	    ...
	    valid_targets = np.concatenate((valid_annotations, 1-valid_annotations), axis=3).astype(np.float32)
            pred, test_loss= sess.run([pred_annotation, loss], feed_dict={image: valid_images, annotation: valid_annotations, targets: valid_targets, keep_probability: 1.0})
